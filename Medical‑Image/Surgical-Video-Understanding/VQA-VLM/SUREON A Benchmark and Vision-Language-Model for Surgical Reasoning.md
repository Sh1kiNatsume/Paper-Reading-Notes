...
个人讲解稿：
1.现有视频标注体系主要是标注（类别标签，人工标注），讲了这是什么，做了什么（低级能力），难以表达手术意图，决策理由，安全判断，未来推测等信息（高级能力），本文就通过专家讲解视频，例如一段视频的转录文本：“由于淋巴结太大，保留邻近分支会影响完整清扫，因此我们决定牺牲该分支。”包含临床意图和决策理由的信息。作者认为把这种视频做成VQA训练有助于模型理解建模手术流程，做推理、安全评估。
2.作者提出了SGM（Semantic Grounding Moment，语义锚定时刻），主要流程：已有专家讲解视频→ 获取或使用对应的时间对齐转录文本
→ GPT-5 生成代理从转录文本中识别候选 SGM→ 根据候选文本对应的时间戳截取视频片段→ GPT-5 验证代理筛选样本→ 生成结构化视频问答数据
SGM 依赖专家讲解，因此存在以下问题：
...
本文包含多智能体设计，但主要用于数据生成与质量验证。作者针对 12 类问题分别设置专门的生成代理和验证代理，先由生成代理根据转录文本生成候选样本，再由验证代理检查视频时间窗口、旁白与画面的对应关系以及答案质量。
